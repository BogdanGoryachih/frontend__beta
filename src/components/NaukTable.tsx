...
import { INauk, IResponseTransactionLoader, ITransaction } from '../types/types'
...
const NaukTable: FC<ITransactionTable> = ({ limit = 3 }) => {
	const { transactions } = useLoaderData() as IResponseTransactionLoader

	const [data, setData] = useState<INauk[]>([])
	const [currentPage, setCurrentPage] = useState<number>(1)
	const [totalPages, setTotalPages] = useState<number>(0)

	const fetchTransactions = async (page: number) => {
		const response = await instance.get(
			`/nauk/pagination?page=${page}&limit=${limit}`
		)
  ...
  {data?.map((transaction, idx) => (
    <tr key={idx}>
      ...
      <td>
        {/* Форма для удаления */}
        <Form method="delete" action="/nauk">
